...
- **Conteúdo:** 8 tutores reais, um card por tutor (`CARD_COUNT = mentors.length`), sem repetição. Isso **encerra a pendência** que constava na seção 7 (adicionar tutores novos para acompanhar a expansão de 4 → 8 formações): os 4 originais (Rodolfo, Linus, Shakira, Steve Jobs) ganharam companhia de Albert Einstein, Michael Jordan, Nikola Tesla e Batman. A fase intermediária em que 8 cards eram montados repetindo os 4 tutores (`mentors[index % mentors.length]`) não existe mais.
- **Rodolfo (`id: 0`) ocupa sempre o slot central do leque**, que é o que já começa selecionado. A ordem do leque é montada explicitamente — o fundador é retirado da lista e inserido no meio, e os demais preenchem os slots ao redor na ordem em que estão declarados. `CENTRE_INDEX` é derivado uma única vez e usado **tanto** pela ordenação **quanto** pela seleção inicial, pra que os dois não possam divergir ao adicionar ou remover um tutor. (Foi exatamente o que quebrou quando os 4 novos entraram: com a montagem antiga por `index % mentors.length`, o slot central passou a cair em quem estivesse no índice 4, e não no fundador.)
